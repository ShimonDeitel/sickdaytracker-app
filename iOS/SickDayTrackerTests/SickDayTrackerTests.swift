...
@MainActor
final class SickDayTrackerTests: XCTestCase {
    ...
    func testAddIncreasesCount() {
        ...
        let added = store.add(SymptomEntry())
        XCTAssertTrue(added)
    ...
    func testFreeLimitBlocksAddWhenReached() {
        ...
        for _ in 0..<(Store.freeLimit + 5) {
            _ = store.add(SymptomEntry())
        }
        let result = store.add(SymptomEntry())
